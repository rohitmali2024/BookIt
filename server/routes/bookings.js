...
router.post("/", verifyToken, async (req, res) => {
  try {
    const { experienceId, firstName, lastName, email, phone, date, time, guests, totalPrice, promoCode, discount } =
      req.body

    // Validate required fields
    if (!experienceId || !firstName || !lastName || !email || !phone || !date || !time || !guests) {
    ...
    const slot = experience.slots.find(
      (s) => new Date(s.date).toDateString() === new Date(date).toDateString() && s.time === time,
    )
